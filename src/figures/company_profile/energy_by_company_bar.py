...
def create_company_profile_bar_plot(df, highlight_company=None):
    """Create a horizontal bar chart of electricity usage by company.

    Parameters
    ----------
    df : pd.DataFrame
        Energy-use dataframe already filtered by year, with ``company_name``,
        ``electricity_usage_kwh``, and ``reporting_scope`` columns.
    highlight_company : str or None
        Company name to visually emphasise (orange ► label via annotation).
    """
    df = df.copy()
    ...
    year = df["reported_data_year"].iloc[0]

    max_value = df["electricity_usage_billions"].max()
    tick_interval = 5 if max_value > 50 else 2

    # Hide the highlighted company's tick label — replaced by an orange annotation
    tick_labels = [
        "" if name == highlight_company else name
    ...
    if highlight_company and highlight_company in companies_order:
        fig.add_annotation(
            text=f"<b>\u25ba {highlight_company}</b>",
            x=0,
            y=highlight_company,
            xref="paper",
            yref="y",
