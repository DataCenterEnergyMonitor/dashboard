...
class FilterManager:
    ...
    def __init__(
        self, app, base_id: str, df: pd.DataFrame, filters: List[FilterConfig]
    ):
    ...
    def _register_callbacks(self):
        ...
        @self.app.callback(outputs, value_inputs, prevent_initial_call=False)
        def update_filter_options(*input_values):
            ...
            for filter_id, value in zip(self.filters.keys(), input_values):
                ...
                if value is not None:
                    ...
                    # Force the default value into input_values list
                    input_values = list(input_values)
                    input_values[list(self.filters.keys()).index(filter_id)] = (
                        filter_config.default_value
                    )
                print(
                    f"Filter {filter_id}: value={value}, default={filter_config.default_value}"
                )
            ...
            for filter_id, filter_config in self.filters.items():
                ...
                should_update = (
                    triggered_filter is None  # Initial load
                    or filter_id == triggered_filter  # This filter was triggered
                    or (
                        filter_config.depends_on
                        and triggered_filter in filter_config.depends_on
                    )  # Dependency changed
                )
                            ...
                            if isinstance(dep_value, list):
                                if "All" not in dep_value:
                                    filtered_df = filtered_df[
                                        filtered_df[
                                            self.filters[dep_filter].column
                                        ].isin(dep_value)
                                    ]
                            elif dep_value != "All":
                                filtered_df = filtered_df[
                                    filtered_df[self.filters[dep_filter].column]
                                    == dep_value
                                ]
    ...
    def _get_filter_options(
        self, filter_config: FilterConfig, filtered_df: pd.DataFrame
    ) -> List[Dict]:
        """Get options for a filter based on the filtered dataframe"""
        unique_values = filtered_df[filter_config.column].dropna().unique()

        # Check if the column contains numeric (integer) data
        is_numeric = pd.api.types.is_numeric_dtype(filtered_df[filter_config.column])

        if is_numeric:
            # Keep numeric values as-is, only convert labels to string
            options = [
            ...
            for filter_config in self.filters.values():
                if filter_config.type == "dropdown":
                    ...
                    filter_components.append(
                        self._create_year_range_pair(filter_config)
                    )
    ...
    def _create_dropdown(self, config: FilterConfig) -> html.Div:
        ...
        if config.type == "year_range_pair":
            ...
            return html.Div(
                [
                    html.Label(
                        config.label, style={"fontFamily": "Inter", "fontWeight": "500"}
                    ),
                    html.Div(
                        [
                            # From Year dropdown
                            html.Div(
                                [
                                    dcc.Dropdown(
                                        ...
                                        value=config.default_value.get(
                                            "from", min_year
                                        ),
                                        placeholder="From",
