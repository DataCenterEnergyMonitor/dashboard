...
def create_reporting_bar_plot(df: pd.DataFrame) -> go.Figure:
    ...
    grouped_df = (
        df.groupby(["reported_data_year", "reporting_scope", "reporting_status"])
        .agg({"company_name": "nunique"})
        .reset_index()
        .rename(columns={"company_name": "num_companies"})
    )

    # Calculate dynamic bar width settings based on number of years
    ...
    for scope in REPORTING_SCOPE_COLORS:
        ...
        if not scope_pending.empty:
            ...
            # Create semi-transparent version for pending
