...
def create_gp_choropleth_map(filtered_df, filtered_geo_df, filters_applied=False, 
    ...
    # 1. Background Layer
    country_geo_df = filtered_df.groupby('country_iso_code')['unique_per_country'].max().reset_index()
    ...
    # 2. State Outlines
    state_geo_df = filtered_df.dropna(subset=['state_iso_code']).drop_duplicates('state_iso_code')
    ...
    state_bubbles = filtered_geo_df.dropna(subset=['state_lat', 'state_lon', 'unique_per_state']).drop_duplicates('state_iso_code')
    city_bubbles = filtered_geo_df.dropna(subset=['lat', 'lon', 'unique_per_city']).drop_duplicates(['lat', 'lon'])

    if not state_bubbles.empty:
        fig.add_scattergeo(
            lat=state_bubbles['state_lat'], lon=state_bubbles['state_lon'],
            text=state_bubbles['state_province'],
            name="State",
            marker=dict(size=state_bubbles['unique_per_state'], color='rgba(255, 165, 0, 0.75)',
                        sizemode='area', sizeref=2. * max(filtered_df['unique_per_state'].fillna(0)) / (25.**2),
                        line=dict(width=1, color='white'))
        )

    if not city_bubbles.empty:
        fig.add_scattergeo(
            lat=city_bubbles['lat'], lon=city_bubbles['lon'],
            text=city_bubbles['city'],
            name="City",
            marker=dict(size=city_bubbles['unique_per_city'], color='rgba(230, 0, 0, 0.85)',
                        sizemode='area', sizeref=2. * max(filtered_df['unique_per_city'].fillna(0)) / (15.**2),
                        line=dict(width=1, color='white'))
        )
